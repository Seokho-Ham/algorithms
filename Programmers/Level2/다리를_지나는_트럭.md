...
function solution(bridge_length, weight, truck_weights) {
  ...
  let trucks = truck_weights.slice(); //트럭들의 목록
  let sum = 0; //현재 트럭들의 무게의 합
  let passed = []; //지나간 트럭
  let passing = []; //다리위에 있는 트럭

  while (passed.length !== truck_weights.length) {
    answer++;
    if (passing.length) {
      if (passing[0].time + bridge_length === answer) {
        passed.push(passing[0].value);
        sum -= passing[0].value;
        passing.shift();
      }
    }
    if (sum + trucks[0] <= weight) {
      sum += trucks[0];
      passing.push({ value: trucks.shift(), time: answer });
    }
  }
